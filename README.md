...
## OM Hospital — סימולטור חדר מיון (`hospital/`)

תיקיית `hospital/` היא **PWA** עצמאית בעברית (RTL) המדמה מערכת ED/EMR מקצועית של
בית החולים "OM Hospital": התחברות מבוססת-תפקיד עם אימות דו-שלבי מדומה, קבלה
וטריאז' עם חישוב דחיפות אוטומטי, תיק קליני, הזמנת בדיקות (CPOE) עם תוצאות מדומות,
מרשמים עם נעילת אלרגיה, לוח בקרה בזמן אמת, ומחולל מכתב שחרור רשמי להדפסה. כל
הנתונים נשמרים מקומית (`localStorage`) — ללא שרת. לסימולציה ולמידה בלבד.
...
גישת דמו (רופא): שם משתמש `ofek` · סיסמה `Ofek2026!`. פירוט מלא וחשבונות נוספים
ב-[`hospital/README.md`](hospital/README.md).

---

## Ultimate Mixology Hub — מערכת מיקסולוגיה (`mixology/`)

תיקיית `mixology/` היא אפליקציית **React 18 + Vite** עצמאית בעברית (RTL) לעולם
הקוקטיילים, בעיצוב "בר לאונג' אפלולי" (זהב מוברש על שחור-פחם). שתי קטגוריות:
**אנציקלופדיית הקלאסיקות** — מאגר 24 קוקטיילים עם מתכונים מדויקים במ"ל, המלצות
מותג, סוג כוס/קרח/שיטה, וגרף רדאר של פרופיל הטעם; ו**מעבדת האלכימיה** — מנוע
רקיחה דטרמיניסטי (קליינט-סייד) שמרכיב קוקטייל מאפס לפי המלאי הקיים, פרופיל החוויה
המבוקש, וטכניקות בר מתקדמות. כל הויזואליזציות ב-SVG טהור; המלאי נשמר ב-`localStorage`.
...
פירוט מלא ב-[`mixology/README.md`](mixology/README.md). לסימולציה והשראה — רקחו במתינות.
